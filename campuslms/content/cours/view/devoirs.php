			...
			<?php
				...
				if ($data['remise'][0]['date']){
					...
					if ($data['remise'][0]['note'] != ''){
						...
						echo "<br/>Votre devoir a été corrigé le ".$data['remise'][0]['datenote'].". Vous avez obtenu la note de <strong>".($data['remise'][0]['note']*1)."%</strong>";
					}else{
						echo '<a class="color_background btn openInMenuBar" href="'.implode('/',$_GET['qs']).'/upload"><strong>Mettre à jour</strong>Remplacer votre devoir</a>';
					}
				}else if ($data['fichier']['valeur'] > 0) {
					//Indiquer date limite de remise, si dispo
					echo "Vous n'avez pas encore remis ce devoir.";
					echo '<div class="clr"></div>';

					echo '<a class="color_background btn openInMenuBar" href="'.implode('/',$_GET['qs']).'/upload"><strong>Téléverser</strong>Remettre votre devoir</a>';
				}else{
					//Rien à remettre
				}
			?>
