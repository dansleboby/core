...
if ($_SESSION['user_level'] == 'collaborateur'){
	echo '<ul id="options">';
		echo '<li>';
			echo '<a class="btn color_background openInMenuBar" href="cours/'.$_GET['qs'][1].'/ponderation">Modifier la pondération</a>';
		echo '</li>';
	echo '</ul>';
}

if (!$data['locked'] && $data['datefin'] && EXPIRATION_ALERT > 0){
	if (time() > strtotime($data['datefin']." - ".EXPIRATION_ALERT." days")){
		echo '<div class="message color_background">';
			echo '<div class="noImg color_text">!</div>';
			echo '<div class="ctn">';
				echo '<h2>Fin de formation imminante</h2>';
				echo '<p>Cette formation se termine à la date suivante : '.$data['datefin'].'. Les leçons ne seront plus accessibles après cette date.</p>';
			echo '</div>';
...
//		return;
	...
	echo "<div id=\"coursContainer\">";

	$nbGroupe = 0;

	foreach ($data['groupes'] AS $groupe){
		echo "<h3 data-groupe=\"".$groupe['id']."\">".$groupe['nom']."</h3>";
		$nbGroupe++;
	...
	function showCours($id, $i){
		...
		?>
		<article class="color_border leconbtn<?php echo (($lecon['disabled'] || $data['locked'])?' disabled':''); ?>">
			<?php
				if ($lecon['disabled'] || $data['locked']){
					echo '<div class="iconLocked"></div>';
				}
			...
			<header>
				...
				<a<?php echo (($lecon['disabled'] || $data['locked'])?' class="openInMenuBar"':''); ?> href="cours/<?php echo $_GET['qs'][1]; ?>/lecon/<?php echo $lecon['id']; ?>">
					Leçon #<?php echo $i; ?>
					<strong><?php echo $lecon['nom']; ?></strong>
					<?php
//						echo $lecon['nb']."_".$lecon['noteFinale']."-".$lecon['notfound'];
					?>
				</a>
			</header>
			<?php
...
//					if (rand(0,1)){
					...
					if ($lecon['nb'] > 0 && $lecon['noteFinale'] > 0){
						echo '<div class="starContainer"><div style="width:'.(floor($lecon['noteFinale']/10)*10).'%;"></div></div>';
					}
				}
			?>
		</article>
	...
	if ($nbDisabled > 0){
		echo '<div class="clr"></div>';
		echo '<p>Cliquez sur une leçon vérouillée pour découvrir ses préalables.</p>';
	}
	...
	if ($data['locked']){
		echo "<a href=\"".$_GET['data']."acces\" class=\"openInMenuBar btn color_background\"><strong>Demander accès</strong> à cette formation</a>";
	}
?>
