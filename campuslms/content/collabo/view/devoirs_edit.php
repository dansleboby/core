...
<form method="post" action="<?php echo $_GET['data']; ?>">
	<h1><?php echo (($_GET['4'] > 0)?'Modifier':'Nouveau'); ?> devoir</h1>
	<div class="clr"></div>
<!--	<label for="titre">Nom</label>!-->

	<input type="text" class="text" placeholder="nom" name="titre" id="titre" value="<?php echo escape($data['fichier']['nom']); ?>"/>

	<textarea class="specialField" data-specialType="wysiwyg" placeholder="Description du devoir" name="description" id="description"><?php echo $data['fichier']['description']; ?></textarea>
...
<?php
	if ($_GET['qs'][5] > 0){
		...
		if ($data['fichier']['etat'] == "recu") {
			...
			echo "&mdash; Aucun fichier lié à ce devoir &mdash;";
		}
	}else{
...
?>
	...
	<?php
		if ($_GET['qs'][5] > 0){
			...
			echo '<a class="delete openInMenuBar" href="'.$link.'">Supprimer le devoir</a>';
		}
	?>
</form>