	...
	if ($_SESSION['user_level'] == 'collaborateur'){
		if ($_GET['qs'][5] == 'nouveau'){
			...
			if ($_POST['titre']){
				...
				//Insert file
				$req = "INSERT INTO cours_lecon_fichiers SET id_lecon='".$lid."', id_upload='".$_POST['fileRef'.$_POST['fichieruploadNb']]."', type='tp', nom='".$_POST['titre']."', description='".$_POST['description']."', valeur='".$_POST['valeur']."', date=NOW()";
				$mysqli->query($req);
				...
				manageUpload("fichier", "data/cours/".$cid."/lecons/".$lid."/tp/".$fid."/fichier", $_POST['fileName'.$_POST['fichieruploadNb']]);
			...
			if ($_GET['qs'][6] == 'delete'){
				...
				if (in_array($_POST['confirmkey'],$keys) && getpassword($_SESSION['user_id']) == getpassword($_SESSION['user_id'], $_POST['confirm'])){
					...
					if ($_POST['ajax']){
					//Confirm text
						$data['confirmText'] = "Le TP a été supprimé.";
						$data['updateDOM'] = array(array(
						    'target'=>'#tpId'.$_GET['qs'][5],
						    'action'=>'remove')
						);
//						$data['refreshContent'] = true;
						exit(json_encode($data));
	...
	//				saveLog('tryDelLecon',$_GET['qs'][2]);
				...
				if ($_POST['titre']){
					...
					if ($_POST['ajax']){
					//Confirm text
						$data['confirmText'] = "Le TP a été mis à jour.";
						$data['updateDOM'] = array(array(
						    'target'=>'#tpId'.$_GET['qs'][5],
						    'action'=>'update',
						    'value'=>"<strong>".$_POST['titre']."</strong>TP")
						);
//						$data['refreshContent'] = true;
						exit(json_encode($data));
					}
